...
function sysCall_init()
    ...
    finish_handle =  sim.getObjectHandle('finish')
    ...
    simOMPL.setCollisionPairs(t,{sim.getObjectHandle('eDrone_outer'),sim.getObjectHandle('hoop_visible3'),sim.getObjectHandle('base_of_hoop_40cm'),sim.getObjectHandle('base_of_hoop_100cm'),sim.getObjectHandle('hoop_visible1')})


    ------------Add the path planning task initial details------------------
    no_of_path_points_required = 15
...
function start_position_callback(msg)
    --print(msg)
    x = 0.099403*msg.data[1]  --0.1033
    y = -0.099403*msg.data[2]
    z = ((msg.data[3]-38.5)*((3.2)/(-1*38.5)))
    sim.setObjectPosition(start_handle,-1,{x,y,z})
    sim.setObjectPosition(finish_handle,-1,{x,y,0.2})
...
--------- Converting from real to whycon ------------------------
        ...
        pose.position.y = -1*(path[i+1]*9.68)
...
function sysCall_actuation()
 ...
 ----------- To choose which path to compute based on goal_id --------------------------------------
        if goal_id == 0 then
            goal_pose = getpose(front_handle3,-1)
            start_pose = getpose(drone_handle,-1)
            --goal_pose = getpose(initial_waypoint_handle,-1)
            --start_pose = getpose(goal_1_handle,-1)
        end
        if goal_id == 1 then
            goal_pose = getpose(back_handle3,-1)
            start_pose = getpose(front_handle3,-1)
        end
        -- if goal_id == 2 then
        --     goal_pose = getpose(start_handle,-1)
        --     start_pose = getpose(back_handle3,-1)

        -- end
        if goal_id == 2 then
            goal_pose = getpose(back_handle1,-1)
            start_pose = getpose(back_handle3,-1)

        end

        if goal_id == 3 then
            goal_pose = getpose(front_handle1,-1)
            start_pose = getpose(back_handle1,-1)
        end


        if goal_id == 4 then
            goal_pose = getpose(start_handle,-1)
            start_pose = getpose(front_handle1,-1)
        end

        if goal_id == 5 then
            goal_pose = getpose(finish_handle,-1)
